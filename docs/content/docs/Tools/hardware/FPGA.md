...
如果你已经安装好了 UHD，那么就只需要从第三步开始，下载镜像并烧录。下载镜像文件有两种方法，第一种就是参考里面的方法，在 [网址](https://files.ettus.com/binaries/uhd_stable/latest_release/) 当中选择所需要的版本，如果你是 uhd 4.1.0，那就选择 4.1.0 即可。解压下载好的压缩文件，打开文件夹下的目录/host/utils，然后右键选择在终端中打开，输入 ls 即可看到所有文件，再输入
...
注意在运行之前，先输入`uhd_find_devices`查看 ip 地址；
...
在 <https://files.ettus.com/binaries/cache/x3xx/> 里面选择对应版本的 FPGA 镜像，直接下载，解压后采用非默认位置的烧录命令进行烧录，不过这种方法我还不知道怎么判断所需版本，我是试了两个就可以了。
